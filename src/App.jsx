...
function genDaily() {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    rows.push({
      ...
      cpc: Math.round(329 * (0.8 + Math.random() * 0.4)),
    })
  }
  return rows
}
...
const CAMPAIGNS = [
  {
    id: 1,
    name: 'Interacción - Perfil IG - Aumento Seguidores',
    status: 'ACTIVE',
    budget: 200,
    spend: 665,
    impressions: 23316,
  ...
  {
    id: 2,
    name: 'Prospección - Calzado Nike / Adidas',
    status: 'ACTIVE',
    budget: 800,
    spend: 29100000,
    impressions: 2800000,
  ...
  {
    id: 4,
    name: 'Retención - Clientes Frecuentes',
    status: 'PAUSED',
    budget: 400,
    spend: 16000000,
    impressions: 1500000,
...
const ADS = [
  {
    id: 1,
    name: 'Presentación / Imagen / Nike V2K Combinan Con Todo',
    type: 'imagen',
    spend: 88000,
    impressions: 551,
    clicks: 13,
  ...
  {
    id: 2,
    name: 'Presentación / Video / TT - VO041 - Unboxing',
    type: 'video',
    spend: 273000,
    impressions: 1532,
    clicks: 42,
  ...
  {
    id: 3,
    name: 'Catálogo / Carrusel / New Balance 574 Serie',
    type: 'carrusel',
    spend: 145000,
    impressions: 980,
    clicks: 31,
  ...
  {
    id: 4,
    name: 'Story / Video / Puma x Vans Flash Sale',
    type: 'video',
    spend: 159000,
    impressions: 2100,
    clicks: 61,
...
function OverviewSection() {
  const weekly = useMemo(() => {
    const wks = [[], [], [], []]
    DAILY.forEach((d, i) => wks[Math.floor(i / 7)].push(d))
    return wks.map((w, i) => ({
      week: `Sem ${i + 1}`,
      spend: Math.round(w.reduce((s, d) => s + d.spend, 0) / w.length),
      ctr: parseFloat((w.reduce((s, d) => s + d.ctr, 0) / w.length).toFixed(2)),
    }))
  }, [])

  const eventsData = [
    { name: 'Add to Cart', value: GLOBAL.add_to_cart },
    { name: 'Checkout', value: GLOBAL.initiate_checkout },
    { name: 'Compras', value: GLOBAL.purchases },
  ]
            ...
            return (
              <div key={f.label} style={{ marginBottom: 10 }}>
                ...
                <div style={{ background: 'rgba(255,255,255,.06)', borderRadius: 3, height: 5 }}>
                  <div style={{ width: `${Math.max(f.pct, 2)}%`, height: 5, borderRadius: 3, background: colors[i] }} />
                </div>
              </div>
            )
          })}
            ...
            <ResponsiveContainer width={120} height={120}>
              <PieChart>
                <Pie data={eventsData} dataKey="value" innerRadius={35} outerRadius={55} paddingAngle={3}>
                  {eventsData.map((_, i) => <Cell key={i} fill={PIE_COLORS[i]} />)}
                </Pie>
              </PieChart>
            </ResponsiveContainer>
            <div style={{ flex: 1 }}>
...
function CampaignsSection() {
  ...
  const sorted = useMemo(() => {
    return [...CAMPAIGNS].sort((a, b) => (a[sortCol] - b[sortCol]) * sortDir)
  }, [sortCol, sortDir])
  ...
  const tdStyle = { padding: '9px 12px', borderBottom: `1px solid rgba(255,255,255,.04)`, color: '#ccc', fontSize: 12, whiteSpace: 'nowrap' }
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 700 }}>
            <thead>
              <tr>
                ...
                <th style={{ ...thStyle('spend'), textAlign: 'right' }} onClick={() => toggleSort('spend')}>Spend {sortCol === 'spend' ? (sortDir === -1 ? '↓' : '↑') : '↕'}</th>
                ...
                <th style={{ ...thStyle('ctr'), textAlign: 'right' }} onClick={() => toggleSort('ctr')}>CTR {sortCol === 'ctr' ? (sortDir === -1 ? '↓' : '↑') : '↕'}</th>
            ...
            <tbody>
              {sorted.map(c => (
                <tr key={c.id} style={{ cursor: 'default' }}
                  ...
                  <td style={{ ...tdStyle, maxWidth: 220, overflow: 'hidden', textOverflow: 'ellipsis' }}>{c.name}</td>
                  ...
                  <td style={{ ...tdStyle, textAlign: 'right', color: C.green }}>{c.ctr.toFixed(2)}%</td>
...
function AnalyticsSection() {
  const weeklyData = useMemo(() => {
    ...
    return wks.map((w, i) => ({
      week: `Sem ${i + 1}`,
      spend: Math.round(w.reduce((s, d) => s + d.spend, 0)),
      clicks: Math.round(w.reduce((s, d) => s + d.clicks, 0)),
      cpc: Math.round(w.reduce((s, d) => s + d.cpc, 0) / w.length),
      impressions: Math.round(w.reduce((s, d) => s + d.impressions, 0)),
    }))
  }, [])
        ...
        <Card>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <ComposedChart data={weeklyData}>
              ...
              <Line yAxisId="right" type="monotone" dataKey="clicks" name="Clics" stroke={C.blue} strokeWidth={2} dot={{ fill: C.blue, r: 4 }} strokeDasharray="4 3" />
            </ComposedChart>
          </ResponsiveContainer>
        </Card>
      ...
      <Card style={{ marginBottom: 12 }}>
        ...
        <ResponsiveContainer width="100%" height={150}>
          <AreaChart data={DAILY}>
            ...
            <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,.04)" vertical={false} />
            <XAxis dataKey="date" tick={{ fill: C.muted, fontSize: 9 }} tickLine={false} axisLine={false} interval={4} />
            <YAxis tick={{ fill: C.muted, fontSize: 9 }} tickLine={false} axisLine={false} tickFormatter={v => (v / 1000).toFixed(0) + 'K'} />
...
function CreativesSection() {
  const chartData = ADS.map(a => ({
    name: a.name.split('/')[2]?.trim() || a.name.substring(0, 20),
    spend: Math.round(a.spend / 1000),
    ctr: a.ctr,
  ...
  return (
    <div>
        ...
        {ADS.map(a => (
          <Card key={a.id} style={{ padding: 0, overflow: 'hidden' }}>
            <div style={{
              ...
              background: 'rgba(141,96,202,.07)', borderBottom: `1px solid ${C.border}`,
              fontSize: 36,
            }}>
              {a.emoji}
            </div>
            <div style={{ padding: 12 }}>
              ...
              <div style={{ marginTop: 8 }}>
                <span style={{
                  ...
                  background: 'rgba(141,96,202,.15)', color: C.accent, fontSize: 10,
                }}>
                  {a.type}
                </span>
              </div>
      ...
      <Card>
        <CardTitle>Comparativo de creatividades — spend vs CTR</CardTitle>
        <ResponsiveContainer width="100%" height={200}>
...
function BudgetSection() {
  const cumulative = useMemo(() => {
    let acc = 0
    return DAILY.map(d => { acc += d.spend; return { date: d.date, value: acc } })
  }, [])
  ...
  const dailyAvg = Math.round(GLOBAL.spend / 30)
  const pacing = Math.round((GLOBAL.spend / (dailyAvg * 30)) * 100)
        ...
        <Card>
          ...
          <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: 12 }}>
            {[
              ...
              { l: 'Spend total', v: fmtCOP(GLOBAL.spend) + ' COP' },
              { l: 'Ingresos estimados (ROAS)', v: fmtM(GLOBAL.spend * GLOBAL.roas) + ' COP' },
            ].map(r => (
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', background: '#0d0d0d', overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        <div style={{ marginTop: 'auto', padding: '16px', fontSize: 10, color: '#444', wordBreak: 'break-all' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{
              background: 'rgba(141,96,202,.15)', border: '1px solid rgba(141,96,202,.3)',
              borderRadius: 4, padding: '3px 8px', fontSize: 11, color: '#8d60ca',
            }}>Tennispremium</span>
            <span style={{
              display: 'flex', alignItems: 'center', gap: 6,
              background: '#161616', border: '1px solid rgba(255,255,255,.07)',
              borderRadius: 6, padding: '5px 10px', fontSize: 12, color: '#ccc',
            }}>
              📅 Mar 16 – Abr 14, 2026
            </span>
          </div>
